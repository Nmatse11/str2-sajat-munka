...
@Component({
  selector: 'app-math-card',
  templateUrl: './math-card.component.html',
  styleUrls: ['./math-card.component.scss']
})
export class MathCardComponent implements OnInit {
  ...
  this.categoryService.getAll().subscribe(category => category.filter(category => category.name === 'math')
    .map(category => {
