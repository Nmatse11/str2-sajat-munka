...
@Component({
  selector: 'app-poetry-card',
  templateUrl: './poetry-card.component.html',
  styleUrls: ['./poetry-card.component.scss']
})
export class PoetryCardComponent implements OnInit {

  title!: string;
  category!: string;
  colorClass: string = 'card-header-orange';
  ...
  this.categoryService.getAll().subscribe(category => category.filter(category => category.name === 'poetry')
    .map(category => {
