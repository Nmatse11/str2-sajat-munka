...
@Component({
  selector: 'app-disney-card',
  templateUrl: './disney-card.component.html',
  styleUrls: ['./disney-card.component.scss']
})
export class DisneyCardComponent implements OnInit {
  ...
  //A kategórihoz tartozó elérhető kérdések, nehézségi szintek frissítése
    this.categoryService.getAll().subscribe(category => category.filter(category => category.name === 'disney')
    .map(category => {
